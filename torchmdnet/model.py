...
class TorchMD_Net_Ext(nn.Module):
    ...
    def reset_parameters(self):
        ...
        if self.prior_model is not None:
            for prior in self.prior_model:
                prior.reset_parameters()#pyright: ignore[reportCallIssue]
        for m in self.extra_output_models.values():
            m.reset_parameters()#pyright: ignore[reportCallIssue]
    ...
    def forward(
        ...
        if self.prior_model is not None:
            for prior in self.prior_model:
                x = prior.pre_reduce(x, z, pos, batch, extra_args)#pyright: ignore[reportCallIssue]
        ...
        if self.prior_model is not None:
            for prior in self.prior_model:
                y = prior.post_reduce(y, z, pos, batch, box, extra_args)#pyright: ignore[reportCallIssue]
